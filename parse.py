...
def parse_profile(t):
    ...
    if per_info:
        ...
        if infos and len(infos) == 3:
            comma = '\uff1a'
            gender = infos[0].text_content()
            item['gender'] = gender[gender.find()+6:]

            intro = infos[1].text_content()
            item['intro'] = intro[intro.find()+6:]

            date = infos[2].text_content()
            item['date'] = date[date.find()+6:]
